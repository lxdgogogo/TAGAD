...
def main(args):
    ...
    pretrain = AnomalyClip(feature.shape[1], last_hidden_size, feature.shape[1], num_layers, dropout, norm, activation, alpha,
                           device).to(device)
    # model train
    ...
    for epoch in epoch_iter:
        ...
        epoch_iter.set_description(f'Epoch {epoch}: train_loss: {loss}')
        if pre == False:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', type=float, default=0.8)  # 300
